...
- **Professional Iconography Standardization (Mar 2026)**: Systematically replaced emojis with Material icons for all UI elements (buttons, headers, navigation, cards) across the entire application. This maintains a professional and consistent visual hierarchy while reserving emojis for avatars and social content. Updated the `DuoButton` widget and all section header helpers to support `IconData`.
