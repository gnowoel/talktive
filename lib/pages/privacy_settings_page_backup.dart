...
class _PrivacySettingsPageState extends State<PrivacySettingsPage> {
  ...
  Future<void> _loadPrivacyInfo() async {
    ...
    try {
      // Always load privacy info to show consent status
      final validation = await _consentService.validateConsentForAds();
      // Check current ad permissions
      final canRequestAds = validation.canRequestAds;
      final needsAction = validation.action != 'none';
      final consentStatus = await _consentService.getConsentStatus();
      final statusMessage = _getStatusMessage(consentStatus);
      final blockInfo = await _adRequestHelper.checkConsentBlocking();
