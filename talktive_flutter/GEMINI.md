...
**Duolingo-Inspired UI/UX**: The app features a clean, dynamic, and playful design inspired by Duolingo's aesthetic. The previous glassmorphism design has been completely replaced with:
...
- **Bottom Navigation**: Floating pill-shaped bar with emoji + text labels, colored background pills for active state, haptic feedback
- **Component Library**: New `lib/widgets/duo/` directory with reusable Duolingo-style components:
  - `DuoButton`: Gradient button with haptic feedback and scale animations
  - `DuoCard`: Clean white card with subtle shadow
  - `DuoAvatar`: Avatar with gradient ring and floor badge
  - `DuoInput`: Modern text input with rounded corners
  - `DuoEmptyState`: Emoji with circular gradient background and CTA
  - `DuoHeader`: Screen header with emoji and title
  - `DuoStatCard`: Stat display with gradient icon circle
- **Screens Redesigned**: All five main screens follow the Duolingo aesthetic:
  - **Plaza**: Public chat with emoji header, clean message bubbles, floating input
  - **Moments**: Card-based photo feed with full-screen creation modal
  - **Chats**: Empty state with "Find Friends" CTA (implementation pending)
  - **Groups**: Empty state with "Create Group" CTA (implementation pending)
  - **Profile**: Gradient header, stat cards grid, achievements section
