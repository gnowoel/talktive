...
Future<void> backgroundMessagingHandler(RemoteMessage message) async {
  ...
  if (activeVersion == null) {
     debugPrint('BackgroundMessaging: No active version selected, ignoring notification until app is opened.');
     return;
  }
  
  if (msgVersion == 'serverpod') {
    if (activeVersion == 'serverpod') {
      debugPrint('BackgroundMessaging: Dispatching serverpod message to Serverpod handler');
      ...
      debugPrint('BackgroundMessaging: Ignoring serverpod message because active version is $activeVersion');
    }
  } else {
    // Treat messages WITHOUT 'serverpod' appVersion as 'firebase' (legacy)
    if (activeVersion == 'firebase') {
      debugPrint('BackgroundMessaging: Dispatching legacy message to Messaging handler');
      await LegacyMessaging.handleMessage(message);
    } else {
      debugPrint('BackgroundMessaging: Ignoring legacy message because active version is $activeVersion');
    }
  }
}
