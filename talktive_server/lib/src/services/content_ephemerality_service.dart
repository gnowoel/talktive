...
class ContentEphemeralityService {
  ...
  static Future<Map<String, int>> runCleanup(Session session) async {
    session.log(
      'TALKTIVE: Starting scheduled content ephemerality cleanup...',
      level: LogLevel.info,
    );
    ...
    try {
      ...
      final totalCleaned = results.values.fold(
        0,
        (prev, element) => prev + element,
      );
      session.log(
        'TALKTIVE: Content cleanup completed. Total ephemeral items removed: $totalCleaned',
        level: LogLevel.info,
      );
    } catch (e, stack) {
      session.log(
        'TALKTIVE: CRITICAL ERROR during content cleanup: $e\n$stack',
  ...
  static Future<int> _cleanupPlazaMessages(Session session) async {
    ...
    final oldMessages = await protocol.Message.db.find(
      session,
      where: (t) =>
          (t.channelId.equals(plazaChannel.id!)) & (t.createdAt < cutoff),
  ...
  static Future<int> _cleanupLoungeMessages(Session session) async {
    ...
    final loungeChannels = await protocol.Channel.db.find(
      session,
      where: (t) =>
          t.type.equals(protocol.ChannelType.lounge) &
  ...
  static Future<int> _cleanupPrivateMessages(Session session) async {
    ...
    final cleanupSet = candidateIds
        .where((id) => !channelIdsWithPersistence.contains(id))
        .toSet();
  ...
  static Future<int> _cleanupMoments(Session session) async {
    ...
    final deleted = await protocol.Moment.db.deleteWhere(
      session,
      where: (t) => t.id.inSet(ids),
    );
    return deleted.length;
  ...
  static Future<Map<String, int>> getCleanupStats(Session session) async {
    ...
    if (plazaChannel != null) {
      stats['archivablePlazaMessages'] = await protocol.Message.db.count(
        session,
        where: (t) =>
            (t.channelId.equals(plazaChannel.id!)) &
            (t.createdAt < now.subtract(plazaMessageLifetime)),
      );
    }
    ...
    if (loungeChannels.isNotEmpty) {
      stats['archivableLoungeMessages'] = await protocol.Message.db.count(
        session,
        where: (t) =>
            (t.channelId.inSet(loungeChannels.map((c) => c.id!).toSet())) &
            (t.createdAt < now.subtract(loungeMessageLifetime)),
      );
    }
    ...
    // Notifications
    stats['archivableReadNotifications'] = await protocol.UserNotification.db
        .count(
          session,
          where: (t) =>
              (t.read.equals(true)) &
              (t.createdAt < now.subtract(readNotificationLifetime)),
        );
    ...
    stats['archivableResolvedReports'] = await protocol.Report.db.count(
      ...
      where: (t) =>
          ...
          (t.resolvedAt < now.subtract(resolvedReportLifetime)),
    );
  ...
  static Future<void> _deleteMessageMedia(
    Session session,
