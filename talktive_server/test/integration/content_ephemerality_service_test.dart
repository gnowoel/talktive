...
void main() {
  withServerpod('Given ContentEphemeralityService', (
    sessionBuilder,
    endpoints,
  ) {
    ...
    group('_cleanupPrivateMessages', () {
      ...
      test(
        'retains old messages for users with keepPrivateChats = true',
        () async {
          final session = sessionBuilder.build();
      ...
      test(
        'retains old messages for users in grace period (expired < 14 days ago)',
        () async {
          final session = sessionBuilder.build();
          ...
          testUser.premiumTrialExpires = DateTime.now().subtract(
      ...
      test(
        'cleans up old messages for users whose grace period expired (> 14 days ago)',
        () async {
          final session = sessionBuilder.build();
