...
void main() {
  withServerpod('Given MessagingService', (sessionBuilder, endpoints) {
    ...
    group('validateMessage', () {
      ...
      test('throws TalktiveException when user is muted', () async {
        ...
        expect(
          ...
          throwsA(isA<protocol.TalktiveException>().having((e) => e.code, 'code', 'USER_MUTED')),
        );
      });
      ...
      test('throws FLOOR_RESTRICTION for media on Floor 1 in Plaza', () async {
        ...
        expect(
          ...
          throwsA(isA<protocol.TalktiveException>().having((e) => e.code, 'code', 'FLOOR_RESTRICTION')),
        );
      });
      ...
      test('throws PREMIUM_REQUIRED for voice message from non-premium user', () async {
        final session = sessionBuilder.build();
    ...
    group('Private Chat Privacy', () {
      test('throws PRIVACY_RESTRICTED when sender is blocked by recipient', () async {
        final session = sessionBuilder.build();
        ...
        final savedChannel = await protocol.Channel.db.insertRow(session, privateChannel);
        ...
        expect(
          ...
          throwsA(isA<protocol.TalktiveException>().having((e) => e.code, 'code', 'PRIVACY_RESTRICTED')),
        );
      });
    });
  });
