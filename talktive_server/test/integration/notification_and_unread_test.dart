...
void main() {
  withServerpod('Notification and Unread Count Integration', (sessionBuilder, endpoints) {
    ...
    setUp(() async {
      ...
      // Create test User A
      userA = await protocol.Resident.db.insertRow(session, protocol.Resident(
      ...
      // Create test User B
      userB = await protocol.Resident.db.insertRow(session, protocol.Resident(
      ...
      privateChannel = await protocol.Channel.db.insertRow(session, protocol.Channel(
    ...
    group('Unread Counts', () {
      test('increases unread count for recipient when sender sends a message', () async {
        final session = sessionBuilder.build();
        ...
        await protocol.Message.db.insertRow(session, protocol.Message(
          channelId: privateChannel.id!,
      ...
      test('resets unread count when marked as read', () async {
        ...
        await ChannelService.markAsRead(session, privateChannel.id!, userB.userInfoId);
    ...
    group('Notification Payloads (Deep Linking)', () {
      test('sendMessageNotification generates correct route for private chat', () async {
        final session = sessionBuilder.build();
      ...
      test('sendAchievementNotification generates correct route and saves to history', () async {
        final session = sessionBuilder.build();
      ...
      test('sendChatInviteNotification generates correct route with channelId', () async {
        final session = sessionBuilder.build();
        ...
        final invite = notifications.firstWhere((n) => n.type == 'chat_invite');
