...
void main() {
  withServerpod('Messaging Recall Sync Integration', (sessionBuilder, endpoints) {
    late protocol.Resident testUser;

    setUp(() async {
      final session = sessionBuilder.build();
      // Use a consistent UUID for predictability
      final userUuid = UuidValue.fromString('a1b2c3d4-e5f6-4a5b-8c9d-0e1f2a3b4c5d');
    ...
    test('Lounge lastMessageAt syncs back to previous message after recall', () async {
      final session = sessionBuilder.build();
      ...
      expect(currentLounge.lastMessageAt!.isBefore(tLatest), isTrue,
        reason: 'lastMessageAt should have moved back to historical message time');
    });
    ...
    test('PrivateChat lastMessage syncs back after recall', () async {
      ...
      final otherUserId = UuidValue.fromString('b2c3d4e5-f6a7-4b8c-9d0e-1f2a3b4c5d6e');
      ...
      await MessagingService.onMessageSaved(session, message: msg1, channel: channel, sender: testUser);
      ...
      await MessagingService.onMessageSaved(session, message: msg2, channel: channel, sender: testUser);

      // Verify current state
      var currentChat = await protocol.PrivateChat.db.findById(session, chat.id!);
